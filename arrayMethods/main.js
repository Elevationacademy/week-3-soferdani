...
// let olderThen40 = ages.filter( y => y > 40)
// let lsls = function (ages) {
//     return ages>40
// }

// console.log ( olderThen40)
...
// let orngeOnes = vegetables.filter(orenge => orenge.color === "orange")
// console.log(orngeOnes)

// orngeOnes.forEach(toShow => console.log(toShow.name))

// --------------good job by me ! solve with 0 help 
// let people = [
...
// const chackSalary = function (personInfo) {
//     if (personInfo.goodPerformance) {
//         personInfo.salary += 300
//     }
// }

// people.forEach(chackSalary)
...
// let massageInLowerCase = messagesFromDad.map(low => low.toLocaleLowerCase())
// console.log(massageInLowerCase)
...
// let usersNeto = users.map(tempo =>{ return { name: tempo.name , city: tempo.address["city"] } } )
// console.log(usersNeto)
...
//     let lookInThisPost = posts.find(singelPost => singelPost.id === postID)
//     let theRes = lookInThisPost["comments"].find(singelComment => singelComment.id === commentID)
//     console.log(theRes)
// }
...
// if (movies.some(oneMove => oneMove.studio === "Marvel")) {
//     console.log ("Let's go watch some movies")
// } else ( console.log("Let's stay home"))

// if (movies.every(oneMove => oneMove.year <2020)) {
...
let usersWithEmailAndName = users.map(NE =>{ return {email:NE.email, companyName: NE.company.name}})
console.log ( usersWithEmailAndName)


let onlyZipStart5 = users
.filter(oneUser => oneUser.address.zipcode.startsWith("5"))
.map(oneUser => oneUser.id)



        
let onlyNameWithC = users.map(oneUesr => oneUesr.name).filter(oneUesr=> oneUesr.startsWith("C"))
console.log(onlyNameWithC)
        // let onlyZipStart5IDs = onlyZipStart5.map(oneUser => oneUser.id)

// console.log(onlyZipStart5IDs)